...
type SceneVisualsResult = {
  clips: string[];
  beatDurations: number[];
  /** Beats used when fetching clips — same segmentation as voiceover sentences. */
  beats?: SceneBeat[];
  /** Beat index per clip (parallel to clips). */
  clipBeatIndices?: number[];
};
...
async function adoptWikimediaBeatClip(
  ...
  for (let ai = 0; ai < attempts.length; ai++) {
    ...
    try {
      ...
      console.warn(
        `[Pipeline] Wikimedia scene ${scene.index} beat ${beat.index} attempt ${ai + 1}:`,
...
async function adoptAiBeatClip(
  ...
  holdSec: number,
  semanticProfile?: BeatSemanticProfile,
  opts?: { skipVision?: boolean; rescueTier?: boolean }
): Promise<boolean> {
  const rescueTier = Boolean(opts?.rescueTier);
  if (!rescueTier && strictVoiceVisualMatchEnabled()) return false;
  if (rescueTier) {
    if (
      !beatVisualRescueEnabled() ||
      dedup.rescueAiClipsUsed >= beatVisualRescueAiMaxClips(dedup.videoLength)
  ...
  if (
    ...
    !(await beatClipPassesVisionGate(
      ...
      dedup,
      semanticProfile,
      rescueTier ? "rescue_ai" : "ai",
      rescueTier ? beatVisualRescueVisionFloor() : undefined
    )).pass
  ) {
    return false;
  }
  ...
  if (await pushClip(withText, holdSec)) {
    ...
    console.log(
      `[Pipeline] Scene ${scene.index} zin ${beat.index}: ${rescueTier ? "rescue AI" : "AI clip"} (${rescueTier ? dedup.rescueAiClipsUsed : dedup.aiClipsUsed}/${rescueTier ? beatVisualRescueAiMaxClips(dedup.videoLength) : dedup.perf.maxAiClipsPerVideo})`
    );
    return true;
  }
  return false;
...
async function adoptStockBeatClipFallback(
  ...
  const scenePersons = resolveScenePersons(scene, videoTitle, dedup.primaryPerson || undefined);
  const stockVisionFloor = stockOpts?.visionFloor ?? stockClipQualityFloor(dedup.videoLength);
  const stockAdoptSource = stockOpts?.adoptSource;
  ...
  const tryClip = async (
    ...
    recordClipAdopt(
      ...
      beat.text,
      withText,
      stockAdoptSource ?? (source === "Pexels" ? "pexels" : "pixabay"),
      undefined,
      dedup.segmentGeoLock,
      undefined,
      vision.worstScore10 ?? undefined
...
/**
 * Degraded rescue when strict CLIP match found nothing — never leave export blocked for an empty beat.
...
async function rescueBeatVisualWhenEmpty(
  ...
  if (!beatVisualRescueEnabled()) return false;
  if (skipComposeNetworkFetch(dedup, "visual rescue", scene.index, beat.index)) return false;

  const floor = beatVisualRescueVisionFloor();
  console.warn(
    `[Pipeline] Scene ${scene.index} zin ${beat.index}: visual rescue (CLIP floor ${floor}/10) — ` +
      `"${beat.text.slice(0, 50).trim()}…"`
  );
  ...
  if (curatedArchiveOnlyVisuals()) {
    if (
      await adoptBestSimilarBeatClip(
        ...
        semanticProfile,
        null,
        { visionFloor: floor, adoptSource: "rescue_similar" }
      )
    ) {
      return true;
    }
    if (
      ...
      const last = dedup.clipAdoptAudit[dedup.clipAdoptAudit.length - 1];
      if (last && last.sceneIndex === scene.index && last.beatIndex === beat.index) {
        last.source = "rescue_archive";
      }
      return true;
    }
  }
  ...
  if (archivePexelsFallbackEnabled() && canUseLicensedStockBeat(dedup)) {
    if (
      await adoptStockBeatClipFallback(
        ...
        holdSec,
        semanticProfile,
        { visionFloor: floor, adoptSource: "rescue_stock" }
      )
    ) {
      return true;
    }
  ...
  for (let attempt = 0; attempt < 4; attempt++) {
    ...
    if (await pushClip(placeholder, holdSec)) {
      ...
      console.warn(
        `[Pipeline] Scene ${scene.index} zin ${beat.index}: rescue placeholder (geen CLIP-match)`
      );
      return true;
    }
  }
...
async function fetchArchiveSentenceMontage(
  ...
  const beatCap = sceneBeatCapForCadenceForVideo(
    scene.duration,
  ...
  if (usable.length < minNeeded) {
    ...
    for (let bi = 0; bi < beats.length; bi++) {
      ...
      if (fastShort) {
        ...
        if (!clipBeatIndices.includes(beat.index)) {
          await rescueBeatVisualWhenEmpty(
            beat,
            scene,
            workDir,
...
async function _runVideoPipelineInner(
  ...
  console.log(
    ...
    if (ttsWordAlignmentEnabled()) {
      ...
      if (storedTts) {
        visualDedup.ttsSceneBeats = buildTtsSceneBeatMap(
          scenes,
          storedTts,
          // One beat per sentence; cap only by minimum clip length (never by cadence).
          (dur) => maxBeatCapForVisualCadence(Math.max(1, dur))
        );
        console.log(
    ...
    visualDedup.visualBeatsTotal = scenes.reduce(
      (sum, s) => sum + maxBeatCapForVisualCadence(s.duration),
      0
    );
    ...
    if (curatedArchiveOnlyVisuals()) {
      if (isFastShortVideoLength(videoLength)) {
        ...
        const poolMs = fastShortArchivePoolWarmMs();
        try {
          visualDedup.archiveCandidatePool = await withTimeout(
            buildVideoArchiveCandidatePool(topicContext, combinedSceneText, {
              ...
              maxPool: fastShortArchivePoolMax(),
            }),
            poolMs,
            "Archive pool 1-min warm"
          );
        } catch (poolErr) {
          console.warn(
            `[Pipeline] Archive pool 1-min warm skipped (${poolMs / 1000}s cap):`,
            (poolErr as Error).message?.slice(0, 80)
          );
          visualDedup.archiveCandidatePool = [];
        }
        console.log(
          `[Pipeline] Archive pool 1-min: ${visualDedup.archiveCandidatePool.length} candidate(s)`
        );
        try {
          const { backfillClipEmbeddingsWithBudget } = await import("./archiveClipIndexBackfill");
          const prewarmMax = fastShortClipIndexPrewarmMax();
          const prewarmMs = fastShortClipIndexPrewarmMs();
          const warmed = await backfillClipEmbeddingsWithBudget(prewarmMax, prewarmMs, {
            ignoreActiveJobCap: true,
          });
          if (warmed.indexed > 0) {
            console.log(
              `[Pipeline] CLIP index 1-min pre-warm: +${warmed.indexed} archive clip(s)` +
                (warmed.timedOut ? ` (${prewarmMs / 1000}s budget)` : "")
            );
          }
        } catch (warmErr) {
          console.warn(
